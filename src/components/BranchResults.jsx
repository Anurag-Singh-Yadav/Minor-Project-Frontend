...
function BranchResults() {
  ...
  const getData = async (subject) => {
    try {
      const res = await axios.post(
        "http://localhost:4000/getresults/all-courses",
        {
          branchCode: branch,
        }
      );
  ...
  const allotedResultSubjectWise = async (courseCode) => {
    try {
      const res = await axios.post(
        "https://localhost:4000/getresults/course-wise-allotment",
        {
          courseCode,
        }
      );
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-5 w-[90vw] md:w-[60vw] mx-auto my-auto min-h-[80vh]">
          {subOffer.map((item, index) => {
            return (
              <div
                key={index}
                className="flex flex-col gap-3 justify-center items-center shadow-[0_8px_30px_rgb(0,0,0,0.12)] font-semibold py-2"
                
              >
                <span>{item}</span>
                <div className="flex justify-center cursor-pointer items-center gap-2"onClick={() => {
